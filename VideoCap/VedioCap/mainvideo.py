'''
  程序名称：视频拉流及保存程序
  函数说明：主界面MainWindow，可以嵌套或单独使用
  重要返回值：两个录制的视频，默认保存在程序的根目录下
            默认自己端口   无需输入
            默认接收端口1  Y:\视频素材\（2轮）罗小黑战记 去声版.mp4  在第27行可以更改
            默认接收端口2  NULL，在第28行可以更改
  主程序使用方法：直接将界面stack4替换成MainWindow函数即可   import mainvideo    from mainvideo import MainWindow
'''
import sys,time
# 导入图形组件库
import cv2
...
class MainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self):
        ...
        Address1 = "Y:\视频素材\（2轮）罗小黑战记 去声版.mp4"
        Address2 = "NULL"
